...
function Navbar() {
    ...
    useEffect(() => {
        ...
        else if (location.pathname === "/add-food") return setIndex(1);
        else if (location.pathname === "/my-data") return setIndex(2);
        else if (location.pathname === "/profile") return setIndex(3);
        else return setIndex(-1);
    }, [location.pathname]);
    return (
        <div className="w-full h-20 fixed bottom-2 flex justify-center items-center">
            <div className="pt-4 pb-4 pl-4 pr-1 h-12 bg-white flex justify-center items-center gap-6 border-2 border-gray-300 shadow-md rounded-full">
                ...
                <Link to="/add-food"><SquarePlus className={`cursor-pointer hover:text-black hover:scale-125 transition-transform ${index == 1 ? "text-black" : "text-neutral-400"}`} /></Link>
                <Link to="/my-data"><GalleryVerticalEnd className={`cursor-pointer hover:text-black hover:scale-125 transition-transform ${index == 2 ? "text-black" : "text-neutral-400"}`} /></Link>
